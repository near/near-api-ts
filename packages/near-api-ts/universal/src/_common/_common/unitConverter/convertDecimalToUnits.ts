import type { Decimal } from '../../../../types/_common/common';
import { pow10 } from './_common/pow10';

/**
 * Convert a human-readable amount (decimal string) to minimal units (integer string).
 *
 * Example: decimals = 24 (NEAR), "1.23" -> "1230000000000000000000000" (25 digits total, 24 after dot)
 *
 * @param decimal   Positive decimal string with up to `decimals` fractional digits
 * @param decimals  Fractional digits count (1..100)
 * @returns         Positive integer string in minimal units
 */
export const convertDecimalToUnits = (decimal: Decimal, decimals: number): bigint => {
  // Split integer and fractional parts (fractional may be undefined)
  const [integerPartRaw, fractionalPartRaw = ''] = decimal.split('.');
