import { pow10 } from './_common/pow10';

/**
 * Convert minimal units (integer string) to a human-readable amount (decimal string).
 *
 * Example: decimals = 24 (NEAR), "1000000...000" (24 zeros) -> "1"
 *
 * @param units     Positive integer string in minimal units ("yocto", "gas", "wei", etc.)
 * @param decimals  Fractional digits count (1..100)
 * @returns         Decimal string with up to `decimals` digits after the dot
 */
export const convertUnitsToDecimal = (units: bigint, decimals: number): string => {
  const scale = pow10(decimals);
  const isNegative = units < 0n;
  const sign = isNegative ? '-' : '';

  // Integer division gives the whole part; the remainder gives the fractional digits
  ...
  if (fractionalRemainder === 0n) return `${sign}${wholePart.toString()}`;
  ...
  if (fractionalDigits.length === 0) return `${sign}${wholePart.toString()}`;
