...
export const getInnerPublicKey = ({ curve, u8PrivateKey }: InnerPrivateKey) => {
  // ed25519
  if (curve === 'ed25519') {
    const publicKeyU8 = u8PrivateKey.slice(Ed25519.SecretKey);
    return {
      curve,
  ...
  // secp256k1
  const publicKeyU8 = u8PrivateKey.slice(Secp256k1.SecretKey);
